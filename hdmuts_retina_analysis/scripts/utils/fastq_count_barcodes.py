...
# Will generate error when SSH'ing (with -X) into Ubuntu 16.04 servers
# Try using Agg rendering engine instead of X11
import matplotlib
matplotlib.use("Agg")
...
def main(prefix, bc_to_crs):
    ...
    bc_to_crs_df = pd.read_csv(bc_to_crs, sep="\t", header=None, names=["barcode", "label"], index_col=0).squeeze("columns")
    ...
    # Save to file
    bc_to_crs_df.to_csv(f"{prefix}.counts.tsv", sep="\t", float_format="%d")
